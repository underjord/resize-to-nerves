...
create_buildroot_dir() {
    ...
    if [[ -d "$BASE_DIR/patches/buildroot" ]]; then
        # Apply patches
...
# Plug in a final .config variable for external toolchains
# If we're building on aarch64 and are targeting aarch64
if [[ "$DEFCONFIG" == *"aarch64"* ]]; then
    echo "Checking if we are on an aarch64 host..."

    if [[ "$HOST_ARCH" == *"x86_64"* ]]; then
        echo "Not on aarch64, cross-compiling!"
        echo "BR2_TOOLCHAIN_EXTERNAL=y" >> $ABS_BUILD_DIR/.config
    elif [[ "$HOST_ARCH" == *"aarch64"* ]]; then
        echo "On aarch64, NOT cross-compiling!"
        echo "BR2_TOOLCHAIN_EXTERNAL=n" >> $ABS_BUILD_DIR/.config
    fi
fi
